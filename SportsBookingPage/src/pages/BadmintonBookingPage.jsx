...
const BadmintonBookingPage = () => {
    ...
    const handleBooking = async (e) => {
        ...
        try {
            ...
            // Map court selection to court ID (assuming Badminton courts are ID 1-6)
            const courtIdMap = { 'C1': 1, 'C2': 2, 'C3': 3, 'C4': 4, 'C5': 5, 'C6': 6 };
            const courtId = courtIdMap[selectedCourt];
            
            // Format times - assuming time format is HH.00-HH.00 
                ...
                <section className={`p-6 rounded-xl shadow-xl ${cardBackgroundColor} space-y-4`}>
                    ...
                    <form onSubmit={handleBooking} className="space-y-4">
                        ...
                        {isMaxLimitReached ? (
                            ...
                            <button
                                type="submit"
                                className={`w-full ${buttonColor} text-white font-bold py-3 rounded-lg shadow-md hover:bg-[#6699CC] text-lg mt-4 disabled:opacity-50 disabled:cursor-not-allowed`}
                                disabled={loading}
                            >
                                {loading ? 'กำลังจองสนาม...' : 'จองสนาม'}
                            </button>
                        )}
                    </form>
                </section>
