...
const HomePage = () => {
    ...
    // โหลด User + โหลดประวัติการจองจาก API
    useEffect(() => {
        ...
        // ดึงประวัติการจองจาก API
        const fetchBookingHistory = async () => {
            try {
                const response = await bookingsAPI.getHistory();
                if (response.bookings && Array.isArray(response.bookings)) {
                    // แปลงข้อมูลจาก API ให้แสดงผล
                    const formattedHistory = response.bookings.map(booking => ({
                        sport: 'สนาม',
                        court: `คอร์ต ${booking.court_id}`,
                        time: booking.start_time ? new Date(booking.start_time).toLocaleString('th-TH') : 'N/A',
                        status: booking.booking_status || 'จองแล้ว'
                    }));
